...
RUN set -xe  \
    ...
    && mkdir /build \
    && curl https://ftp.postgresql.org/pub/source/v$PG_VERSION/postgresql-$PG_VERSION.tar.bz2 -o /build/postgresql-$PG_VERSION.tar.bz2 \
    && cd /build/ && tar xvf postgresql-$PG_VERSION.tar.bz2 \
    && cd /build/postgresql-$PG_VERSION/contrib/cube && sed -i 's/#define CUBE_MAX_DIM (100)/#define CUBE_MAX_DIM (2000)/' cubedata.h \
